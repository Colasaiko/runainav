...
const template = (art) => {
  return [
    ...
    '              <Link href="' + art.relatedHref + '" className="text-sm font-medium text-brand-600 hover:underline">',
    '                ' + art.relatedText + '',
    '              </Link>',
    '            </div>',
    '          </div>',
    '        </article>',
    '      </main>',
    '      <FloatingBackButton fallbackHref="' + art.fallbackHref + '" />',
    '      <Footer />',
    '    </div>',
    '  );',
    '}'
...
articles.forEach(art => {
  ...
  console.log('Created ' + art.slug + ' with raw JSX content');
});
